...
var dbServer = "ankioci.education.nsw.gov.au";
var dbServerUrl = "https://"+dbServer+"/ords/eduiot";
  ...
  return {
    "parse" : function(carName,address,data,displayName) {
      ...
      // uint8_t     is_clockwise;
        ...
        var clockwise = false;
        if(data.readUInt8(10) == 0x47 || data.readUInt8(10) == 0x48) {
          clockwise = true;
        }
        console.log("Message[0x"+msgId.toString(16)+"][Position Update]: ",data," Location: (",trackLocation,")",trackLocation.toString(16)," id:(",trackId,") ",trackId.toString(16)," type: ",trackType," offset: ",offset," speed: "+speed+" clockwise: ",clockwise);
        if(trackId == 33) {
          //console.log("********** Start");
        ...
        var dateTime = new Date().getTime();
        var args = {
          data: {"eventtype": "speed", "groupid":""+groupId, "deviceid":""+devId,"datetime":""+dateTime,"datetimestr":dateFormat.asString(),"carid":address,"carname":displayName,"location":trackLocation,"trackId":trackId,"offset":offset,"clockwise":clockwise,"speed":speed },
          headers: { "Content-Type": "application/json" },
          requestConfig: {
              rejectUnauthorized: false,
              timeout: 1000, //request timeout in milliseconds 
...
//        console.log("Data length: "+data.length);
        ...
        var lastRecvLaneChangeId=data.readUInt8(8);
        var lastExecLaneChangeId=data.readUInt8(9);
        var lastDesiredLaneChangeSpeedMmPerSec=data.readUInt16LE(10);
        var aveFollowLineDriftPixels=data.readInt8(12);
        var hadLaneChangeActivity=data.readUInt8(13);
        var uphillCounter=data.readUInt8(14);
        var downhillCounter=data.readUInt8(15);
        if(data.length == 19) {
        ...
        console.log("Message[0x"+msgId.toString(16)+"][Track Event]: ",data," from: "+fromTrackId+" To: "+toTrackId+" Left/Right Wheel Distances: "+leftWheelDistance+"/"+rightWheelDistance+" "+trackStyle+crossedStartingLine+" Track Count: "+trackCount[carName]+" lastRecvLaneChangeId:"+lastRecvLaneChangeId+" lastExecLaneChangeId: "+lastExecLaneChangeId+" lastDesiredLaneChangeSpeedMmPerSec: "+lastDesiredLaneChangeSpeedMmPerSec+" aveFollowLineDriftPixels: "+aveFollowLineDriftPixels+" hadLaneChangeActivity: "+hadLaneChangeActivity+" uphillCounter: "+uphillCounter+" downhillCounter: "+downhillCounter);
        ...
        var dateTime = new Date().getTime();
        var args = {
          data: {"eventtype": "tracktransition", "groupid":""+groupId,"deviceid":""+devId,"datetime":""+dateTime,"datetimestr":dateFormat.asString(),"carid":address,"carname":displayName,"fromtrackid":fromTrackId,"totrackid":toTrackId,"leftdistcm":leftWheelDistance,"rightdistcm":rightWheelDistance,"lanemm":lanemm,"uphill":uphillCounter,"downhill":downhillCounter,"trackstyle":trackStyle,"trackid":trackCount[carName]},
          headers: { "Content-Type": "application/json" },
          requestConfig: {
              rejectUnauthorized: false,
              timeout: 1000, //request timeout in milliseconds 
      ...
      else if (msgId == 0x2a) { // ANKI_VEHICLE_MSG_V2C_LOCALIZATION_INTERSECTION_UPDATE
        console.log("Message[0x"+msgId.toString(16)+"][Intersection Event]: ",data);
        console.log("Size: "+data.length);
        var roadPieceIdx=data.readUInt8(2);
        var offset = data.readFloatLE(3);
        var intersectionCode=data.readUInt8(7);
        var isExisting=data.readUInt8(8);
        var mmSinceLastTransitionBar=data.readUInt16LE(9);
        var mmSinceLastIntersectionCode=data.readUInt16LE(11);
        console.log("Road Piece Index: "+roadPieceIdx+", offset: "+offset);
      }
